...
export function MatchesPage() {
  ...
  const loadRealData = async () => {
    try {
      ...
      setError('Falha ao conectar com a API. Verifique sua chave ou conexão.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {matches.length === 0 ? (
          <div className="bg-gray-800 p-10 rounded-xl text-center border border-gray-700">
            ...
            <p className="text-gray-400">Não há jogos ao vivo no momento.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {matches.map((match: Match) => (
              <div 
                key={match.id} 
                className="bg-gray-800 rounded-xl p-5 border border-gray-700 hover:border-green-500/50 transition-all"
              >
                <div className="text-[10px] text-green-400 font-bold uppercase mb-3">
                  {match.league}
                </div>
